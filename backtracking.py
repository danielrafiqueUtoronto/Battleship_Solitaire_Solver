...
def FC(unAssignedVars, csp, allSolutions, trace):
    ...
    if trace: pass
    for val in nextvar.curDomain():
        if trace: pass
        nextvar.setValue(val)
        noDWO = True
        for constraint in csp.constraintsOf(nextvar):
            if constraint.numUnassigned() == 1:
                if FCCheck(constraint, nextvar, val) == "DWO":
                    noDWO = False
                    if trace: pass #print "<==falsified constraint\n"
                    break
        if noDWO:
        ...
        nextvar.restoreValues(nextvar, val)
    nextvar.setValue(None)
...
def FCCheck(constraint, assignedvar, assignedval):
    ...
    var.unAssign() # ADD THIS LINE!
    if var.curDomainSize() == 0:
        return "DWO"
    else:
        return "OK"
    
def GacEnforce(constrs, csp, assignedVar, assignedVal):
    #cstrs is a collection of constraints not known GAC
    #establish GAC on them and on all affected 
    while not len(constrs)==0: #constraints.empty()??
            ...
            for val in var.curDomain():
                if not constraint.hasSupport(var, val):
                    ...
                    for recheck in csp.constraintsOf(var):
                        if recheck != constraint and not recheck in constrs:
                            constrs.append(recheck)
    return "OK"
